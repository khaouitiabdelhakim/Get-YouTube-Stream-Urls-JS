...
async function testHttp403Code(url) {
  try {
    const response = await axiosInstance.head(url); // Send a HEAD request using Axios instance
...
async function decipherSignature(encSignatures) {
  ...
  if (!decipherFunctionName || !decipherFunctions) {
    ...
    try {
      // Fetch the HTML content of the YouTube video page
      const response = await axiosInstance.get(decipherFunctUrl);
      const javascriptFile = response.data;
